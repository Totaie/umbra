...
void SdlInputHandler::sendHostShortcut(short keyCode)
{
    ...
    // modifier flags on the packet. So we have to actually press and release the
    // modifiers around the key instead of just setting the flags.
...
void SdlInputHandler::hideHostCursor()
{
    ...
    // hid the cursor, this will show it again. Ctrl+Alt+Shift+N flips it back.
    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Asking host to stop drawing its cursor");
...
void SdlInputHandler::switchHostDisplay(int displayIndex)
{
    if (displayIndex < 0 || displayIndex >= HOST_MAX_SWITCHABLE_DISPLAYS) {
        SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                    "Ignoring out of range host display index: %d",
                    displayIndex);
        return;
    }

    SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
                "Switching host to display %d",
                displayIndex);
...
void SdlInputHandler::setLocalCursorVisible(bool visible)
{
    ...
    // is still worthwhile so it takes effect if the user toggles to absolute mode.
